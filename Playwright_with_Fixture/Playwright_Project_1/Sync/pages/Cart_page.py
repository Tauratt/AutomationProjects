...
class Cart(Settings):
    ...
    def _get_names_prices(self):
        ...
        self.check_expect_exists(cards, timeout=5000, name="Cart items")

        count = cards.count()

        for i in range(count):
            card = cards.nth(i)
            name = (card.locator(INVENTORY_ITEM_NAME_CART).inner_text()).strip()
            price = (card.locator(INVENTORY_ITEM_PRICE_CART).inner_text()).strip()

            self.products_dict['Cart'].setdefault(name, []).append(price)

    @allure.step("Проверка добавленных товаров в Корзине")
    def check_products_in_the_cart(self):
    ...
    def checkout_button(self):
        ...
        try:
            ...
            self.check_expect_count(button, 1, timeout=5000, name="Cart Checkout Button")
            button.click()
            step('Click Checkout Button')

            title = self.page.locator(title_user_info)
            self.expect(title).to_have_text(word_user_info_page)
    ...
    def input_user_info(self):
        ...
        try:
            ...
            self.check_expect_count(name_field, 1, timeout=5000, name="First Name Field")
            self.check_expect_count(last_field, 1, timeout=5000, name="Last Name Field")
            self.check_expect_count(code_field, 1, timeout=5000, name="Postal Code Field")

            name_field.fill(name)
            last_field.fill(last_name)
            code_field.fill(postal_code)

            # Проверка введённых значений через assert (если не совпало — AssertionError)
            v1 = name_field.input_value()
            v2 = last_field.input_value()
            v3 = code_field.input_value()
    ...
    def click_button_continue(self):
        ...
        try:
            ...
            self.check_expect_count(button, 1, timeout=5000, name="Button Continue")
            button.click()
    ...
    def select_methods(self):
        ...
        self.check_products_in_the_cart()
        checkout_result = self.checkout_button()
        if checkout_result:
            user_info_result =  self.input_user_info()
            if user_info_result:
